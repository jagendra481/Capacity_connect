...
class OTPModel {
  ...
  static async createOTP({ userId = null, email, purpose = 'email_verification' }) {
    ...
    if (db.getIsPgConnected()) {
      ...
      await db.query(
        `INSERT INTO email_verification_otps (user_id, email, otp_hash, purpose, expires_at)
         VALUES ($1, $2, $3, $4, $5)`,
      ...
      db.memoryStore.otps.push({
        ...
        user_id: userId,
        email: cleanEmail,
        otp_code: otpCode, // stored plain in memory for fallback matching
        otp_hash: otpHash,
        purpose,
        attempts: 0,
  ...
  static async verifyOTP({ email, otp, purpose = 'email_verification' }) {
    ...
    if (db.getIsPgConnected()) {
      ...
      if (new Date() > new Date(record.expires_at)) {
        ...
        return { isValid: false, message: 'This verification code has expired. Please request a new code.' };
      }
